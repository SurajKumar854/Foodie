...
class FoodWhaleUserAuthEndpoint extends Endpoint {
  ...
  Future<Response>signup(Session session,FoodWhaleUser foodWhaleUser) async{
    ...
    if(await isEmailExist(session, foodWhaleUser)) {
      return Response(message: "Email is already registered.", status: false);
    }
  
    if(await isPhoneExist(session, foodWhaleUser)) {
  ...
  Future<bool>isEmailExist(Session session,FoodWhaleUser foodWhaleUser) async{
  var emailCount=  await FoodWhaleUser.db.count(session,where:(user){
      return user.email.equals(foodWhaleUser.email);
    } );
     return emailCount > 0 ? true : false;
  }

   Future<bool>isPhoneExist(Session session,FoodWhaleUser foodWhaleUser) async{
    var phoneNoCount=  await FoodWhaleUser.db.count(session,where:(user){
      return ( user.countryCode.equals(foodWhaleUser.countryCode)&user.mobileNo.equals(foodWhaleUser.mobileNo));
    } );
