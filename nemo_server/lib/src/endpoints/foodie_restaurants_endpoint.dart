...
class FoodieRestaurantsRepoEndpoint extends Endpoint {

 Future<Response>signup(Session session,FoodieRestaurants foodieRestaurants) async{
    
    if(await isEmailExist(session, foodieRestaurants.email)) {
      return Response(message: "Email is already registered.", status: false);
    }
  
    if(await isPhoneExist(session, foodieRestaurants.countryCode,foodieRestaurants.mobileNo)) {
      return Response(message: "Phone number is already registered.", status: false);
    }

    foodieRestaurants.password = await Utlis.generateHashPassword(foodieRestaurants.password);
    
     await FoodieRestaurants.db.insertRow(session, foodieRestaurants);

    return Response(message: "Restaurant registered successfully..", status: true);
   
  }

  Future<bool>isEmailExist(Session session,String emailParam) async{
  var emailCount=  await FoodieRestaurants.db.count(session,where:(user){
   ...
   Future<bool>isPhoneExist(Session session,String countryCodeParam,String phoneParam) async{
    var phoneNoCount=  await FoodieRestaurants.db.count(session,where:(user){
...
Future<Response>sendAccountVerifyOTPLink(Session session,String emailParam)async{
  ...
  Future<FoodieRestaurants?>signIn(Session session,String email,String password) async{
    var passwordDecrypt= await Utlis.generateHashPassword(password);
    var user=await FoodieRestaurants.db.find(session, where: (user) {
...
Future<Response>createNewPassword(Session session,String email, String password)async{
  var user = await FoodieRestaurants.db.find(session,where: (p0) {
  ...
  if(user.first==null){
    ...
    FoodieRestaurants.db.updateRow(session,user.first);
